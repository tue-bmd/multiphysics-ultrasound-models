...
def jacobian(inv, z=None, step=1e-3):
    ...
    for i in range(len(z0)):
        ...
        zp[i] += step
        zm[i] -= step
        cols.append((inv.predict(z_to_x(inv, zp))
                     - inv.predict(z_to_x(inv, zm))) / (2 * step))
...
def scaled_jacobian(inv, z=None, step=1e-3):
    """Jacobian divided by the observation noise: J / sigma."""
    return jacobian(inv, z, step) / inv.sigma()[:, None]
...
def fisher(inv, z=None, step=1e-3):
    """Fisher information in standardized coordinates, F = J^T Sigma^-1 J."""
    js = scaled_jacobian(inv, z, step)
    return js.T @ js


def spectrum(inv, z=None, step=1e-3):
...
PROFILE_LEVELS = ("imaging", "imaging_plus_calibration", "posterior")

#: Roles whose prior enters the middle profile level: quantities measured
#: independently of these three observations, which are data in their own right
#: and would appear in a likelihood if the auxiliary experiment were written
#: out.  In this package the only one is the reconstructed-to-true length
#: ``scale``, and it appears only in the calibration sensitivity arm.
#:
#: The point-spread widths are **not** in this list.  They are estimated from
#: the observations in every arm of the main comparison, which is the whole
#: point: the question is whether one window's information about the operator
#: reaches another window's tissue parameter, not whether an external
#: measurement of the operator would help.
AUXILIARY_ROLES = ("calibrated",)
...
#: quantities with a further limit appear here.  Everything absent from this
#: table has no hard limit the model itself imposes and is bounded by the padded
#: catalogue range alone.
PHYSICAL_DOMAIN = {
    # the bolus cannot arrive before the injection, and the observation window
    # ends at twenty seconds
    "t0": (0.0, 20.0),
    # the lesion center and half-width must lie inside the imaged profile
    "x_c": (0.0, 30.0e-3),
    "R": (None, 15.0e-3),
    # a point spread wider than the imaged profile is not a point spread
    "w0": (None, 15.0e-3),
    "w_bmode": (None, 15.0e-3),
    "w_swe": (None, 15.0e-3),
    "w_ceus": (None, 15.0e-3),
    "sig_e": (None, 15.0e-3),
}
...
def physical_bounds(inv, pad_decades=2.0):
    ...
    from ..params import full_catalogue
    m, sd = standardize(inv)
    lo, hi = [], []
    for i, n in enumerate(inv.free):
        e = full_catalogue()[n]                # catalogue, never the override
...
#: sensitivity check below.  A lesion margin five millimetres wide is not a
#: margin; the catalogue range tops out at four.  This is not the bound the
...
#: *tested* against.
PLAUSIBLE_CEILING = {"sig_e": 5.0e-3}
