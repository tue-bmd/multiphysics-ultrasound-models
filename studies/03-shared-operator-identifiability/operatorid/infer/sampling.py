...
def _split(chains):
    """Split each chain in half, which turns a slow drift within a chain into a
    disagreement between chains that Gelman-Rubin can see."""
    m, n, d = chains.shape
    h = n // 2
...
def _rank_normalize(chains):
    """Rank-normalize the pooled draws, then map back to normal scores.

    A rank transform makes the diagnostic invariant to monotone
    reparameterization and finite for heavy tails, which the ordinary statistic
    is not (Vehtari and colleagues, Bayesian Analysis 2021)."""
    ...
    for j in range(d):
        ...
        out[:, :, j] = ndtri((r - 3.0 / 8.0) /
                             (x.size - 0.25)).reshape(m, n)
    return out


def rhat_rank(chains):
    """Rank-normalized split Gelman-Rubin."""
    return gelman_rubin(_rank_normalize(_split(np.asarray(chains, float))))


def ess_bulk(chains):
    """Effective sample size of the rank-normalized draws."""
    return effective_size(_rank_normalize(_split(np.asarray(chains, float))))


def ess_tail(chains):
    """Effective sample size in the tails, as the smaller of the effective
    sizes of the 5 and 95 percent indicator sequences.

    A 90 percent interval is a statement about the tails, so the bulk effective
    size is the wrong number to quote beside it."""
    ...
    for j in range(d):
        ...
        e = []
        for ind in ((x <= q5).astype(float), (x <= q95).astype(float)):
            e.append(effective_size(_split(ind[:, :, None]))[0])
        out[j] = np.nanmin(e)
    return out
